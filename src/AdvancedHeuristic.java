...
public class AdvancedHeuristic implements Heuristic {
	...
	this.visited.clear();
        ...
        //Jika state saat ini adalah goal, maka value heuristik adalah 0
	if (state.isGoal()) {
            return 0;
	}
		
        //Jika state bukan goal, maka harus dicari jumlah langkah minimal
        //untuk mendapatkan nilai heuristik
        return this.getMinimumRequiredMoves(state);
    }
	
    //Menghitung jumlah gerakan minimal
    private int getMinimumRequiredMoves(State state) {
	visited.add(0);
		
	int value = 1; //Initial value
		
        //Untuk setiap mobil, dihitung space yang dibutuhkan untuk bergerak
        //di sisi depan dan belakang
	for (Integer car : getInitialBlockingCars()) {
            ...
            //Value gerakan minimal akan ditambah dengan value blocking dari mobil target
    ...
    //Method ini berfungsi untuk mengidentifikasi mobil yang menghalangi
    private int getBlockingValue(int car, int needsSpaceFront, int needsSpaceBack) {
        //Node mobil saat ini akan dimasukkan ke list visited
        //sehingga tidak akan diperiksa dua kali
	visited.add(car);
